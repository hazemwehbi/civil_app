    ...
    <div class="component-wrap">
        ...
        <v-card class="mt-3">
            <v-card-title>
                ...
                <v-spacer></v-spacer>
                <v-btn :disabled="!checkActive()" @click="create()" style="background-color: #06706d; color: white"
                    class="lighten-1">
                    {{ trans('data.create_contract') }}
                ...
                <template slot="items" slot-scope="props">
                    <td>
                        <div style="display: inline-flex; padding-left: 30%" align="center">
                            ...
                            <v-btn v-if="props.item.status == 'new'" :disabled="!checkActive()" small fab
                                color="success" @click="edit(props.item)">
                                <v-icon color="white">edit</v-icon>
                            </v-btn>
                            <div>
                                <v-btn color="primary" small fab :disabled="!checkActive()"
                            ...
                            <v-btn color="error" :disabled="!checkActive()"
                                v-if="props.item.status == 'new' || props.item.status == 'rejected'" small fab
                    ...
                    <td>
                        <div align="center">
                            <v-chip class="ma-2" :disabled="!checkActive()" v-if="props.item.status != ''"
                                :color="getColor(props.item.status)" text-color="white">
                                {{ trans('data.'+props.item.status) }}
                            </v-chip>
                        </div>
                    </td>
                    <td>
...
export default {
    ...
    data() {
        ...
        return {
            ...
            headers: [
                ...
                {
                    text: self.trans('data.contractors'),
                    value: 'supporters',
                    align: 'center',
                    sortable: false,
                },
    ...
    mounted() {
        const self = this;
        self.$eventBus.$on(
            ['CONTRACT_ADDED', 'CONTRACT_UPDATED', 'CONTRACT_DELETED', 'CONTRACT_ADDED'],
            () => {
                self.loadRequests(() => { });
            }
        );
    ...
    methods: {
        sendRequest(id) {
            ...
            self.$store.commit('showDialog', {
                ...
                okCb: () => {
                    axios
                        .post('/estate_owner/confirm-contract', { id: id })
                        .then(function (response) {
                            if (response.data.success === true) {
                                self.$eventBus.$emit('CONTRACT_UPDATED');
        ...
        edit(item) {
            console.log(item)
            const self = this;
            self.$refs.supportEdit.create(item);
        },
        // viewSupport(item) {
        //     alert(1)
        //     // const self = this;
        ...
        // },
         viewSupport(item) {
            ...
            const self = this;
            this.$refs.supportView.create(item);
        },
        createdDate(date) {
            const current_datetime = new Date(date);
            return current_datetime.toLocaleDateString('en-US');
        ...
        trash(id) {
            ...
            self.$store.commit('showDialog', {
                ...
                okCb: () => {
                    axios
                        .delete('/estate_owner/request-contract/' + id)
                        .then(function (response) {
                            if (response.data.success === true) {
                                self.$eventBus.$emit('CONTRACT_DELETED');
                                self.$store.commit('showSnackbar', {
                                    message: response.data.msg,
                                    color: response.data.success,
                                });
        ...
        loadRequests(cb) {
            ...
            axios
                .get('/estate_owner/request-contract', { params: params })
                .then(function (response) {
                    if (response.data.success === true) {
