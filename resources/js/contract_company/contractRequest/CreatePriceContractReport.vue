    ...
    <v-container justify-center>
        <v-form ref="form" v-model="valid" lazy-validation>
            <v-card-actions justify-left>
                ...
                <v-btn v-if="contractRequest.is_sent == 0" style="background-color: #06706d; color: white"
                ...
                <v-card-text>
                    <v-container grid-list-md>
                        <v-layout row wrap>
                            <v-flex xs12 sm6 md6>
                                <b-card-group deck>
                                    <b-card :header="trans('data.contract_name')" header-tag="header1" title="">
                                        <b-card-text>{{ contract_name }}</b-card-text>
                                    </b-card>
                                </b-card-group>
                            </v-flex>
                            <v-flex xs12 sm6 md6>
                                ...
                                <b-card-group deck>
                                    <b-card :header="trans('data.price')" header-tag="header1" title="">
                                        <vuetify-money v-model="price" v-bind:label="label"
                                            ...
                                            v-bind:disabled="contractRequest.is_sent == 1" v-bind:outlined="outlined"
...
export default {
    ...
    methods: {
        ...
        getSupportServiceDetail() {
            ...
            axios
                .post('contract_company/show-contract-details', data)
                .then(function (response) {
                    ...
                    self.contract_name = result.supporter.name;
                    self.price = result.price;
                    self.contractRequest = result;
        ...
        send() {
            ...
            if (this.$refs.form.validate()) {
                self.loading = true;
                axios
                    .post('contract_company/send-contract-offer', data)
                    .then(function (response) {
                        self.loading = false;
