                ...
                <v-list subheader three-line>
                    ...
                    <template >
                        ...
                        >
                            <v-list-tile-action>
                                <v-layout wrap>
                                    <v-flex xs12 md12 @click="markAsDone(task)">
                                        <v-checkbox v-model="task.is_completed"></v-checkbox>
                                    </v-flex>
                                </v-layout>
                            </v-list-tile-action>
                            <v-list-tile-content @click="view(task)">
                                <v-list-tile-title>
                                    ...
                                    <v-flex v-for="(task, k) in taskLists[0][category.id]" :key="k">
                                        ...
                                        >
                                            ...
                                            >
                                                <v-spacer></v-spacer>
                                                <v-tooltip top>
                                                    <div @click="markAsDone(task)" slot="activator">
                                                        <v-checkbox
...
export default {
    ...
    methods: {
                ...
                axios
                    ...
                    .then(function (response) {
                        ...
                        if (response.data.success === true) {
                            if (self.view_style == 'grid') {
                                tasks = response.data.msg.tasks;
                            } else {
                                tasks = response.data.msg.tasks.data;
                            }
                            self.taskLists = _.concat(self.taskLists, tasks);
                            self.url = _.get(response, 'data.next_page_url', null);
                            self.projectCategories = response.data.msg.project_categories;
        ...
        markAsDone(task) {
            const self = this;
            axios
                .get('/enginner_office/project-tasks/mark-completed', {
                    params: {
        ...
        deleteTask(task) {
            ...
            self.$store.commit('showDialog', {
                ...
                okCb: () => {
                    axios
                        .delete('/enginner_office/project-tasks/' + task.id, {
        ...
        getFilterData() {
            const self = this;
            axios
                .get('/enginner_office/project-tasks/filter-data/' + self.projectId)
                .then(function (response) {
