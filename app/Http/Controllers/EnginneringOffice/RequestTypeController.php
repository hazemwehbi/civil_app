...
class RequestTypeController extends  Controller
{
    ...
    public function getOfficeRequests(Request $request)
    {
        ...
        if (!$user->is_emp) {
            ...
            $id = $user->id;
            $requests = VisitRequest::with('customer', 'project', 'specialties', 'requestEnginners', 'offices','report','report.media')->where('request_type', 'visit_request')->whereHas('requestEnginners', function ($q) use ($id) {
                $q->where('user_id', $id);
            });
            if (!empty($project_id)) {
                $requests = $requests->where('project_id', $project_id);
